...
# dispersion were removed. The upper panels also reveal that MSDKI measures are
# let sensitive to noise artefacts than standard DKI measures (as pointed by
# [NetoHe2018]_), particularly one can appriciate that MSK maps always present
...
# captures the same tissue information but on different scales (but rescaled to
# values between 0 and 1).  It is important to note that SMT model parameters
# estimates should be used with care, because the SMT model was shown to be
# invalid NetoHe2019]_. For instance, although SMT2 parameter f and uFA may be
# a useful normalization of the degree of non-Gaussian diffusion (note than
# both metrics have a range between 0 and 1), these cannot be interpreted as a
# real biophysical estimates of axonal water fraction and tissue microscopic
# anisotropy.
#
#
# References
