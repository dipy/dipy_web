...
# of voxels within the cuboid ROI and that meet the FA threshold constraint.
# This mask can be used to calculate the number of voxels that were kept, or to
# also apply an external mask (a WM mask for example). Second, the function
...
# determined by the peak threshold which gives an upper limit of the ratio of
# the second peak to the first peak) are maintained, and from these voxels a
# new response function is determined. This process is repeated until
...
wm_mask = (np.logical_or(FA >= 0.4, (np.logical_and(FA >= 0.15,
                                                    MD >= 0.0011))))
