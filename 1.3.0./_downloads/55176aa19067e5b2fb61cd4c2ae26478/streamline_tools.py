...
# returns only those streamlines that pass though the ROI. The ROI should be
...
# mask). This function can also exclude all the streamlines that pass though an
# ROI by setting the ``include`` flag to ``False``. In this example we'll
...
# create an ROI mask from that label and create two sets of streamlines, those
# that intersect with the ROI and those that don't.
